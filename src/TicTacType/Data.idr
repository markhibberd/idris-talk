...
move : {board : Board} -> (position : Position) -> (player : Player) -> (game : Game board) ->
     {default ItIsJust prf : (IsJust (tryValidMove position player board))} -> Game (runMove $ ValidMove board)
...
-- Give me the previous board
--previousBoard' : Game b -> Maybe Board
--previousBoard' start = Nothing
--previousBoard' (load _) = Nothing
--previousBoard' (move m g) = Just . toBoard $ g

--data IsInProgress : Game (runMove b m) -> Type where
--  ItIsInProgress : IsInProgress {b} (move m g)

{--
previousBoard : (g: Game b) -> {default move prf : Game b} -> Board
previousBoard g {prf} with (g)
  previousBoard g {prf = move} | move _ gg = toBoard gg
--}

-- take back a move, if and only if it is possible
